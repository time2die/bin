...
# Download serverlist
curl -s -k https://www.hidemyass.com/vpn-config/l2tp/ > $serverlist
...
function updatenow
{
		...
		# Download hosted script version to temp file
                curl -s -k https://hmastuff.com/linux/hma-openvpn.sh > /tmp/hma-openvpn.sh
...
hmaserverip=$(grep "$hmaservername" $serverlist | awk '{ print $1 }')
...
wget -O /tmp/hma-template.ovpn http://zdcdn.hidemyass.com/other/hma-template.ovpn >/dev/null 2>&1
